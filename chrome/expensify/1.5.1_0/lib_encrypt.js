// A namespace for the encryption tools
var g_encryptionUtilities = {

    _encrypt: function( plaintext, iv, key ){
        // see source of http://josh-davis.org/ecmaScrypt
        var inputData   = ecmaScrypt.encode_utf8( plaintext );
        var keyAsNumber = ecmaScrypt.toNumbers( key );
        var ivAsNumber  = ecmaScrypt.toNumbers( iv );
        
        var ciph = ecmaScrypt.encrypt(
            inputData,
            ecmaScrypt.modeOfOperation.CBC,
            keyAsNumber,
            keyAsNumber.length,
            ivAsNumber
        ); 
    ...
    /**
     * Return the SS0 token
     * 
     * @param String partnerPassword   Password provide by Expensify
     * @param String partnerUserSecret Password generated by your App
     * @param String IV                IV give by Expensify
     * @param String AES_KEY           KEY given by Expensify
     */
    getSSO: function( partnerPassword, partnerUserSecret, IV, AES_KEY){
        var ssoData = {
            "arandom"          : Math.floor( Math.random() * 10000000 ),
            "expires"          : Math.floor( (new Date().getTime()/1000) + 60*30 ),
            "partnerPassword"  : partnerPassword,
            "partnerUserSecret": partnerUserSecret
        };
        var ssoTxt = JSON.stringify( ssoData );
        
        return g_encryptionUtilities._encrypt(ssoTxt, IV, AES_KEY);
    }
}
 ...
 */
// source http://josh-davis.org/ecmaScrypt
var ecmaScrypt = {
